...
def evaluate(args, models, tokenizers, prefix=""):
    all_results = []
    all_examples = []
    all_features = []
    eval_dataloaders = []
    ...
    # As we use multiple model together, the evaluated example should consistent between different models
    ...
    for index, model in enumerate(models):
        if args.n_gpu > 1 and not isinstance(model, torch.nn.DataParallel):
            models[index] = torch.nn.DataParallel(model)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for i, example_index in enumerate(example_indices):
            ...
            all_no_logits = []
            all_unk_logits = []
            for outputs in all_out_put:
                output = [to_list(output[i]) for output in outputs]
                start_logits, end_logits, yes_logits, no_logits, unk_logits = output
                all_start_logits.append(start_logits)
                all_end_logits.append(end_logits)
                all_yes_logits.append(yes_logits)
                all_no_logits.append(no_logits)
                all_unk_logits.append(unk_logits)

            result = CoqaResult(unique_id=unique_id,
                        start_logits=torch.mean(torch.as_tensor(all_start_logits),dim=0),
                        end_logits=torch.mean(torch.as_tensor(all_end_logits),dim=0),
                        yes_logits=torch.mean(torch.as_tensor(all_yes_logits),dim=0),
                        no_logits=torch.mean(torch.as_tensor(all_no_logits),dim=0),
                        unk_logits=torch.mean(torch.as_tensor(all_unk_logits),dim=0))
...
def main():
    ...
    tokenizers = []
    models = []
    for index, model_name_or_path in enumerate(args.model_name_or_path):
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        logger.info("Loading multiple models %s for evaluation",
                    args.model_type)
        logger.info("Evaluate the following checkpoints: %s", args.model_name_or_path)
        global_step = ""
        _, result = evaluate(args, models, tokenizers, prefix=global_step)
